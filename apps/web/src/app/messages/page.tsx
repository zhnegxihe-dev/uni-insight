...
import { getConversationList } from "@/lib/social";
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      ...
      <div className="flex items-center gap-2">
        ...
        {totalUnread > 0 && (
          <span className="rounded-full bg-accent px-2 py-0.5 text-xs font-medium text-white">{totalUnread} 未读</span>
        )}
      </div>
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map((item) => {
            const schools = safeParse<string[]>(item.other.verifiedSchools, []);
            return (
              <Link key={item.id} href={`/messages/${item.id}`} className="card block px-4 py-3 transition hover:border-zinc-300">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 truncate text-sm text-zinc-500">
                      {item.lastMessage
                        ? `${item.lastMessage.senderId === user.id ? "我：" : ""}${item.lastMessage.content}`
                        : "开始聊天"}
                    </p>
                  </div>
                  <div className="shrink-0 text-right">
                    {item.lastMessage && <p className="text-xs text-zinc-400">{formatRelative(item.lastMessage.createdAt)}</p>}
                    {item.unread > 0 && (
                      <span className="mt-1 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-accent px-1.5 text-xs font-medium text-white">
                        {item.unread}
                      </span>
                    )}
                  </div>
