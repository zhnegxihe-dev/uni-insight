...
/** 推广池：受商家委托、明示标注的推广内容。与信任池（经验帖/避雷帖）物理隔离。 */
export default async function PromoPage() {
  const where: Prisma.ExperiencePostWhereInput = { status: { not: "hidden" }, postType: "promo" };
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            这里是商家委托的推广内容，均已明示标注，不进入经验帖/推荐流。内容仍接受点赞、收藏与监督。
          </p>
        </div>
        <Link href="/posts" className="btn-ghost whitespace-nowrap">
          返回经验帖
        </Link>
      </div>
